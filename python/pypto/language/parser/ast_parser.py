...
class ASTParser:
    ...
    def _merge_cache_policy_attr(
        ...
        Placed right after ``dump_vars`` and before task and cross-core attrs
    ...
    def _parse_optimizations_list(  # noqa: PLR0912 -- each public optimization has one parse branch
        self,
        value: ast.expr,
        *,
        owner: str = "pl.at",
        ...
        ``pl.split(MODE)`` sets the cross-core split mode,
        ``pl.cross_core_slot(slot_num=N)`` sets the automatic-pipe slot count,
        and repeated ``pl.cross_core_pipe(...)`` entries describe independent
        planner-selected physical FIFOs.
    ...
    def _try_parse_pl_cross_core_pipe(
        ...
        rows = self._eval_pipe_int_literal(shape_node.elts[0], "valid_shape[0]", minimum=1)
        cols = self._eval_pipe_int_literal(shape_node.elts[1], "valid_shape[1]", minimum=1)
        ...
        tensor_id = self._eval_pipe_int_literal(
            values["tensor_id"], "tensor_id", minimum=0, maximum=(1 << 63) - 1
        )
        slot_size_bytes = self._eval_pipe_int_literal(
            values["slot_size_bytes"], "slot_size_bytes", minimum=1, maximum=(1 << 31) - 1
        )
        slot_num = self._eval_pipe_int_literal(
            values["slot_num"], "slot_num", minimum=1, maximum=(1 << 31) - 1
        )
        if slot_size_bytes > ((1 << 31) - 1) // slot_num:
            raise ParserSyntaxError(
                "pl.cross_core_pipe slot_size_bytes * slot_num must not exceed "
                f"{(1 << 31) - 1} bytes, got {slot_size_bytes * slot_num}",
                span=self.span_tracker.get_span(node),
            )
        return (
            tensor_id,
            ...
            slot_size_bytes,
            slot_num,
            self._eval_pipe_int_literal(values["pipe_id"], "pipe_id", minimum=0, maximum=(1 << 31) - 1),
            self._eval_pipe_int_literal(values["bundle"], "bundle", minimum=-1, maximum=(1 << 31) - 1),
        )

    def _eval_pipe_int_literal(
        self,
        value: ast.expr,
        field: str,
        *,
        ...
        if (
            ...
            literal_value = -value.operand.value
        if literal_value is None:
            raise ParserSyntaxError(
                f"pl.cross_core_pipe {field} must be an integer literal",
                span=self.span_tracker.get_span(value),
            )
        if literal_value < minimum:
            requirement = "positive" if minimum == 1 else "non-negative" if minimum == 0 else f">= {minimum}"
            raise ParserSyntaxError(
                f"pl.cross_core_pipe {field} must be {requirement}, got {literal_value}",
                span=self.span_tracker.get_span(value),
            )
        if literal_value > maximum:
            raise ParserSyntaxError(
                f"pl.cross_core_pipe {field} must not exceed {maximum}, got {literal_value}",
                span=self.span_tracker.get_span(value),
            )
    ...
    def _merge_forward_sticky_dump(
        ...
        for i, (k, v) in enumerate(new_attrs):
            if k == "dump_vars":
                ...
                return new_attrs
        # Insert ``dump_vars`` before task and trailing cross-core attrs so a
        # print -> reparse cycle reproduces the same positional attr order.
        insert_at = next(
            (
                i
                for i, (k, _) in enumerate(new_attrs)
