...
L0TileResult ChooseL0TileLocal(const L0TileConfig& cfg) {
  // 1. Validate inputs.
...
L0TileResult ChooseL0Tile(const L0TileConfig& cfg) {
  // tile.matmul_bias can impose schedule-specific N bounds for its auxiliary
  // Bias storage. The shared Fusebox descriptor does not encode those bounds
  // yet, so preserve main's exhaustive local chooser for precisely that case.
  // Ordinary matmul plans continue to use the shared model/emit contract.
  if (cfg.max_n != 0 || cfg.max_n_pipelined != 0 || cfg.max_n_nested_pipelined != 0) {
    return ChooseL0TileLocal(cfg);
  }
