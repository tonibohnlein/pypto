...
using cross_core_pipe::BuildExplicitPipeSetup;
using cross_core_pipe::ExplicitCrossCorePipe;
...
using ExplicitBoundaryPipes = std::unordered_map<const Stmt*, ExplicitCrossCorePipe>;
...
std::pair<int64_t, int64_t> GetStaticBoundaryShape(const TypePtr& type, const Span& span) {
  ...
  CHECK_SPAN(tile && tile->shape_.size() == 2, span)
      << "Explicit cross-core pipe boundary requires a rank-2 TileType";
  auto rows = cross_core_pipe::TryGetNonNegativeConstInt(tile->shape_[0]);
  auto cols = cross_core_pipe::TryGetNonNegativeConstInt(tile->shape_[1]);
  CHECK_SPAN(rows.has_value() && cols.has_value(), span)
      << "Explicit cross-core pipe boundary requires a static tile shape";
  return {*rows, *cols};
}

ExplicitBoundaryPipes MatchExplicitPipesToBoundaries(
    const std::vector<ExplicitCrossCorePipe>& pipes, const std::vector<const Stmt*>& boundary_order,
    const std::map<const Stmt*, CVBoundaryMove>& boundary_moves, const Span& span) {
  CHECK_SPAN(pipes.empty() == boundary_order.empty(), span)
      << "Cross-core pipe contract and emitted boundaries disagree";

  ExplicitBoundaryPipes matched;
  matched.reserve(boundary_order.size());
  size_t pipe_index = 0;
  std::vector<bool> pipe_seen(pipes.size(), false);
  bool started = false;
  ...
  for (size_t boundary_index = 0; boundary_index < boundary_order.size(); ++boundary_index) {
    ...
    // Mutually exclusive first-matmul/matmul-acc uses of the same SSA source
    // share one FIFO. A distinct source advances the ordered protocol. After
    // the last descriptor, start the next protocol cycle: this is how one
    // logical pipe is reused by a rolled loop and its separately emitted tail.
    ...
    if (logical_source != nullptr && (source != logical_source || boundary.direction != logical_direction)) {
      pipe_index = (pipe_index + 1) % pipes.size();
    ...
    CHECK_SPAN(pipe.direction == expected_direction, span)
    ...
    CHECK_SPAN(pipe.valid_rows == full_rows && pipe.valid_cols == full_cols, span)
    ...
    CHECK_SPAN(source_bytes.has_value() && destination_bytes.has_value(), span)
        << "Cross-core pipe boundary has a dynamic slot size";
    const int64_t full_bytes = std::max(*source_bytes, *destination_bytes);
    CHECK_SPAN(pipe.slot_size_bytes == full_bytes, span)
    ...
    pipe_seen[pipe_index] = true;
  }
  CHECK_SPAN(!started || std::all_of(pipe_seen.begin(), pipe_seen.end(), [](bool seen) { return seen; }),
             span)
      << "Cross-core pipe contract contains a descriptor that has no emitted boundary";
  CHECK_SPAN(!started || pipes.size() == 1 || pipe_index == pipes.size() - 1, span)
      << "Cross-core pipe boundary sequence ends partway through its ordered protocol";
  return matched;
}
...
std::vector<StmtPtr> BuildCoreBody(CoreSide side, const std::vector<StmtPtr>& stmts,
                                   ...
                                   const ExplicitBoundaryPipes& explicit_boundary_pipes,
    ...
    if (bm_it != boundary_moves.end()) {
      {
        ...
        auto explicit_it = explicit_boundary_pipes.find(stmt.get());
        const std::optional<int> pipe_id = explicit_it == explicit_boundary_pipes.end()
                                               ? std::nullopt
                                               : std::optional<int>(explicit_it->second.pipe_id);
    ...
    if (affinity == keep_affinity || affinity == CoreAffinity::SHARED) {
      ...
      if (auto for_stmt = std::dynamic_pointer_cast<const ForStmt>(stmt)) {
        auto new_body = BuildCoreBody(side, FlattenBody(for_stmt->body_), stmt_map, boundary_moves,
                                      explicit_boundary_pipes, tpop_var_remap, superseded_tpop_vars,
        ...
        if (else_body.has_value()) {
          auto new_else_stmts =
              BuildCoreBody(side, FlattenBody(*else_body), stmt_map, boundary_moves, explicit_boundary_pipes,
...
ExpandedKernel ExpandMixedFunction(const FunctionPtr& func, bool create_group = true) {
  ...
  std::optional<std::vector<ExplicitCrossCorePipe>> explicit_pipes;
  ExplicitBoundaryPipes explicit_boundary_pipes;
  if (func->HasAttr(kCrossCorePipePlanAttr)) {
    explicit_pipes =
        cross_core_pipe::DecodeExplicitCrossCorePipes(func->GetAttr<std::string>(kCrossCorePipePlanAttr));
    CHECK_SPAN(explicit_pipes.has_value(), func->span_)
    ...
    if (func->HasAttr("slot_num")) {
      ...
      for (const auto& pipe : *explicit_pipes) {
        CHECK_SPAN(pipe.slot_num == scope_slot_num, func->span_)
            << "Cross-core pipe " << pipe.pipe_id << " slot count " << pipe.slot_num
            << " disagrees with outlined cross_core_slot " << scope_slot_num;
      }
    }
    explicit_boundary_pipes =
        MatchExplicitPipesToBoundaries(*explicit_pipes, boundary_order, boundary_moves, func->span_);
  }
  ...
  CHECK_SPAN(!explicit_pipes.has_value() || (gm_sync_pushes.empty() && gm_sync_pops.empty()), func->span_)
  ...
  auto aic_stmts = BuildCoreBody(CoreSide::AIC, stmts, stmt_map, boundary_moves, explicit_boundary_pipes,
  ...
  auto aiv_stmts = BuildCoreBody(CoreSide::AIV, stmts, stmt_map, boundary_moves, explicit_boundary_pipes,
  ...
  auto pipe_setup = explicit_pipes.has_value()
                        ? BuildExplicitPipeSetup(func->name_, aic_name, aiv_name, *explicit_pipes, aic_final,
                                                 aiv_final, func->span_)
                        : BuildAutomaticPipeSetup(func->name_, aic_name, aiv_name, aic_final, aiv_final,
                                                  slot_num_override, func->span_);
  aic_final = PrependPipeSetup(pipe_setup.aic_stmts, aic_final);
  ...
  aiv_final = PrependPipeSetupKeepingSubblockIdx(pipe_setup.aiv_stmts, aiv_final);
  ...
  const auto emitted_attrs = StripAttr(func->attrs_, kCrossCorePipePlanAttr);
