...
class TopDownRetargeter {
 ...
 private:
  ...
  void RetargetBranchProducer(const VarPtr& var, const StmtPtr& branch, const MemRefPtr& target,
    ...
    RetargetAssign(var, def->second, target, memory, /*liveness_stop_at=*/nullptr,
  ...
  /// while reads later in the same arm are still observed. `check_liveness`
  /// skips that check entirely; Buffer IR branch-storage legalization sets it
  /// false because its target is a fresh allocation the access index has never
  /// seen. The op-legality checks below always apply.
  bool RetargetAssign(const VarPtr& var, const VarDef& def, const MemRefPtr& target,
                      std::optional<MemorySpace> target_memory, const Stmt* liveness_stop_at = nullptr,
    ...
    if (check_liveness && !IsTargetDeadAtAssign(def, target, liveness_stop_at)) return false;
    PlanRewrite(var, target, target_memory);
    return true;
  }
  ...
  bool RetargetIfReturn(const VarPtr& var, const VarDef& def, const MemRefPtr& target,
                        std::optional<MemorySpace> target_memory, const Stmt* liveness_stop_at) {
