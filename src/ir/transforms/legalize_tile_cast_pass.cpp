...
ExprPtr MakeCast(const ExprPtr& x, DataType to, int mode,
                 const std::vector<std::pair<std::string, std::any>>& attrs, const Span& span) {
  std::vector<std::pair<std::string, std::any>> kw = {{"target_type", to}, {"mode", mode}};
  auto cast = OpRegistry::GetInstance().Create("tile.cast", {x}, kw, span);
  return std::make_shared<Call>(cast->op_, cast->args_, cast->kwargs_, attrs, cast->GetType(), cast->span_);
}
 ...
 public:
  ...
  StmtPtr VisitStmt_(const AssignStmtPtr& op) override {
    ...
    for (size_t i = 0; i + 1 < chain.size(); ++i) {
      ExprPtr cast_expr = MakeCast(cur, chain[i], mode, call->attrs_, op->span_);
    ...
    final_assign->value_ = MakeCast(cur, chain.back(), mode, call->attrs_, op->span_);
