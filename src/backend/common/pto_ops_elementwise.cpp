...
static bool EmitFragmentedTcvt(const CallPtr& op, codegen::PTOCodegen& codegen,
  ...
  const std::string config = " " + config_attr;

  // PTOAS's non-saturating narrowing helper takes a scratch tile. Its no-tmp
  // overload instead reaches for a fixed TMP_UB_OFFSET region that PyPTO's
  // level3 allocator never reserves, so dropping the operand here would let a
  // fragmented cast scribble over a live tile. Resolve the scratch once, ahead
  // of the row loop so it dominates every fragment, and hand the whole tile to
  // each one: TcvtScratchCapacityBytes caps its width at a single 64-column
  // block, so one scratch over-provisions any fragment this loop emits.
  INTERNAL_CHECK_SPAN(op->args_.size() == 1 || op->args_.size() == 2, op->span_)
      << "Internal error: tile.cast takes src[, tmp], but got " << op->args_.size() << " arguments";
  std::string tmp_ssa;
  std::string tmp_type;
  if (op->args_.size() == 2) {
    auto tmp_tile = As<ir::TileType>(op->args_[1]->GetType());
    INTERNAL_CHECK_SPAN(tmp_tile, op->args_[1]->span_)
        << "Internal error: tile.cast tmp operand must be a TileType";
    RequireStaticValidShapeForPtoas(tmp_tile, "tile.cast", "tmp", op->args_[1]->span_);
    tmp_ssa = EnsureStaticViewTileSsa(op->args_[1], codegen, "tcvt_tmp_view");
    tmp_type = GetTileViewTypeAnnotation(op->args_[1], codegen);
  }
  ...
  for (int64_t col = 0; col < cols->value_; col += *fragment_width) {
    ...
    const std::string ins_operands = tmp_ssa.empty() ? src_view.first : src_view.first + ", " + tmp_ssa;
    const std::string ins_types = tmp_ssa.empty() ? src_view.second : src_view.second + ", " + tmp_type;
    codegen.Emit("pto.tcvt ins(" + ins_operands + config + " : " + ins_types + ") outs(" + dst_view.first +
                 " : " + dst_view.second + ")");
    if (!valid_cols) {
...
static std::string MakeTcvtCodegenPTO(const CallPtr& op, codegen::CodegenBase& codegen_base) {
  ...
  if (EmitFragmentedTcvt(op, codegen, config_attr)) return "";
