...
字节数与数量、pipe ID 和协议 bundle（`-1` 表示未分 bundle）。每个条目都是一个 `pl.CrossCorePipe`
描述符，其 `direction` 为 `pl.CrossCoreDirection`（`CUBE_TO_VECTOR` 或 `VECTOR_TO_CUBE`）。`tensor_id` 和 `bundle`
