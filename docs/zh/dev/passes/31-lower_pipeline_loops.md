...
**cube 累加器默认是唯一的例外——它们不被打标记。** 流水线 stage 会对它*加载*的操作数做多缓冲：这些加载与上一 stage 的计算重叠，因此各 stage 的操作数缓冲确实同时存活，必须保持分离。累加器由串行化 cube 写入，所以默认的 drain-before-next 调度无论操作数流水线多深都只需一块 L0C。带 `pipeline_double_buffer_c=true` 的循环是显式例外：其 cube Acc 定义先携带完整源 stage，随后 `CanonicalizeIOOrder` 在 `MemoryReuse` 前把该 Acc membership 对 2 取模，因此即使 `F > 2`，每个完整复制组也只有两个 L0C stage residue。该标记正是让这两块缓冲保持分离的全部机制——语句顺序不会被改变，因为普通的 stage 优先顺序本就把每个 drain 排在其自身 MAD 之后。AutoTile 的自动识别器要求迭代数能被 `F` 整除，从而避免单独分配尾组。外层 pipeline 仍不会给该累加器追加 membership。生产者算子判定仍很重要：以 Acc 为目标的非 MAD 数据搬运（例如 `tile.extract(..., target_memory=Acc)`）是真正的 per-stage 缓冲，继续保留普通的完整 stage membership。
