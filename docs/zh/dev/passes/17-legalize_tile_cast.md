...
后端还可声明原生转换的安全列分片宽度。此布局约束由 **PTO 代码生成**处理，
而不是本 Pass：单行分片直接写入最终目标的子视图 (subview)，保留源和目标的
父缓冲区行跨度 (row pitch)。不分配紧密分片临时缓冲区，不发出重组 `tmov`，
不增加 GM 请求，也不改变舍入模式；结果保留原 `valid_shape`。

Ascend910B 的 `INT32→FP16` 和 `FP16→INT8` 分片最多 128 个元素。无填充的完整宽度
`32`、`64`、`128`、`256`、`896` 保留原生路径；`224` 按行分成 `128 + 96`。
物理 `[32,128]`、有效 `[16,112]` 等列填充情况也采用单行视图，避免带掩码的
跨行重复转换。运行时有效列数逐分片裁剪，空分片受条件保护，循环仅访问有效行。
该路径增加循环与指令开销，但不增加 tile 存储；性能模型需计价实际降级序列。
真实 PTOAS 测试覆盖两种内存归属模式；数值正确性仍需设备上的逐转换与量化链回归验证。
`valid_shape` 之外的存储内容未定义，因此这些数值 oracle 会明确忽略该区域。
