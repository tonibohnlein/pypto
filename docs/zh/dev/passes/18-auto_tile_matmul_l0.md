...
6. **M/N 切分（当 `m < M` 或 `n < N`）** —— `[M, N]` 输出 Acc 的物理占用超过 L0c。
   ...
   **前端规范的 split-K 归约**被匹配为三个相邻部分：一个 `tile.create([M, N])` 全输出累加器占位值；一个循环携带该值、完成 K 归约的 pipeline；以及一个 2D 输出 store。其中的归约本身接受**两种源写法**：
   ...
   两个不同的操作数必须来自循环内直接的 GM→Mat load，且静态 shape 与 valid shape 都覆盖完整矩形面板；除此之外循环只能含标量地址计算。本 pass 把输出网格移到源 K-loop 外：对每个 `(mi, ni)` 创建合法的 `[m, n]` Acc，克隆完整 K-loop，并把两次 load 收窄到该输出窗口。`dbC=1` 逐个完成并 drain 归约；`dbC=2` 按 `reduction_i, reduction_{i+1}, drain_i, drain_{i+1}` 成对发射，使两个 Acc 结果同时存活，并允许第一个 drain 与第二个归约的 cube 工作重叠。奇数个输出 tile 的最后一块单独 drain。改写后的循环保留源使用的写法，因此两种写法产生相同的输出网格、相同的收窄 load 和相同的 store 链，只在那一条归约语句上不同。随后普通的调用级 AutoTile 改写会继续对收窄后的调用做必要的内部 K 切分——一旦触发，peel 写法会在分支后产生*两个*内层 K-loop，而谓词写法只产生一个，其谓词为 `init_cond and ko == 0`。该顺序不可颠倒：只切片已有的完整 Acc 仍然需要不可能的 `[M, N]` L0C 分配。

   `dbC=2` 的两个累加器是持久种子：后续输出 pair 交替复用同两个 allocation root，而不是为每个输出 tile 创建 InsertSync 可见的新 root。带边界 tile 的网格使用单 Acc chooser，因为一对循环携带种子必须共享统一的物理与 valid 类型。
...
对于规范 split-K 链，同一输出网格包围的是完整的**源 K 归约**，而不是切片最终 Acc。单 Acc（`dbC=1`）方案会先完成并 drain 当前输出 tile，再开始下一个 tile。双缓冲（`dbC=2`）方案则先成对发射两个完整源 K 归约，再发射对应的两个 drain，从而在不交错或改变归约语义的前提下实现双 Acc ping-pong。每个缩窄归约内部的普通 call 改写仍会选择合法的内层 K 分块。Issue #2232 中，逻辑 INT32 `[16, 1152]` 结果在 Ascend910B 上的物理占用为 `32 × 1152 × 4 = 144 KiB`，因此需要沿 N 切分。每个生成的 N 子块都会运行全部八个源 K block；dbC 决定这些已完成的归约是逐个 drain 还是成对 drain。
