...
为计入发射/设置效应，`C_load` 还会对每个动态 A/B extract 加上分别标定的开销，`C_mad` 也可对每个串行 `TMATMUL_ACC` continuation 加上依赖开销。这些系数在对应后端完成标定前默认为零。
