...
`C_load` 是所选循环序下 L1→L0A/L0B 的操作数流量，按 `GetL0CostModel()` 给出的各 buffer 带宽缩放（op-sim 实测标定：L0A 约为 L0B 的 1.5 倍，而非 datasheet 的 2 倍）；`C_mad` 是 cube MAD 代价（每条 `TMATMUL` 的发射开销 × K-fractal 数）。`C_drain` 是 FIXPIPE 的 L0C 回写，**按每个输出 tile 计费**：`⌈M/m⌉·⌈N/n⌉ · (drain_fixed + bytes_c·m·n/bw_drain + m·drain_penalty·(odd(⌈n/N0⌉)−1))`。前两项是设备验证过的对齐模型（固定发射开销 + 按字节的吞吐）。第三项是**非对齐**修正：FIXPIPE 每次只处理 `N1 M1 M0 N0` FRACTAL_NZ 累加器的一个 M-行，并把 `N1 = ⌈n/N0⌉` 个 N-fractal（`N0 = 32/bytes_c`）按 2 的幂对齐分组并行 burst，因此非 2 的幂的 fractal 数会把**奇部** `odd(N1)−1` 串行成额外的 pass，每个 M-行按 `drain_penalty` 计费（即 N%32 的 drain 悬崖，例如 `n=80 → odd(10)=5`）。由于 drain 数为 `⌈M/m⌉·⌈N/n⌉`，**拆分输出（M/N）会增加 drain 数，而拆分 K 不会**（部分和在单块 L0C 上累加，每个 `(m,n)` 块只回写一次）—— 这既阻止 chooser 在浅 K 形状上过度拆分 M/N，非对齐项也把非对齐-N tile 正确定价为 drain-bound 从而不被过度选择（设备 FIXPIPE 实测：仅字节模型对非对齐-N drain 低估最多约 3.5×，例如 `320×320` 选中 `160×80`）。对齐 tile（`odd(N1)=1`）与之前的字节模型完全一致。搜索对每个 `(m, n)` 的**所有**合法 `k` 都穷举（不是只取最大合法 k —— 当 `kt ≠ align_k` 时 `⌈K/k⌉·⌈k/kt⌉` 关于 `k` 非单调）。wall 平局时按 `(padded_compute, ⌈K/k⌉, C_load, …)` 字典序决出；其中 `C_load` 键在 MAD-bound 的 `(m,n)`↔`(n,m)` 平局中挑出隐藏 load 更低的那一侧（L0B 带宽更慢，故 m-block 更少者更省）。
