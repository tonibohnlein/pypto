...
- any function call, task submission, synchronization, cache-maintenance, or unknown builtin in the loop subtree declines residency because preheader motion could cross an unknown or hidden ordering effect between iterations; other direct control-flow or effect statements before the candidate close the hoistable prefix;
