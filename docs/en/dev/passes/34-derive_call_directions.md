...
**Manual-scope deps are a separate layer.** A `Submit` carries its `deps=[...]` edges in the first-class `deps_` field. The legacy attrs encoding — `Call.attrs["manual_dep_edges"]` (a `vector<VarPtr>` of `Scalar[TASK_ID]` / `Array[N, TASK_ID]` entries) — is what `SubmitToCallView` synthesises from `deps_` for predate-Submit consumers, and is also the shape hand-built post-derive test fixtures use. `DeriveCallDirections` reads and writes only `arg_directions`; the later `ExpandManualPhaseFence` pass may rewrite a consumer's full TaskId-array dep to a dummy-barrier TaskId (preserving the consumer's kind: a `Submit` stays a `Submit`).
