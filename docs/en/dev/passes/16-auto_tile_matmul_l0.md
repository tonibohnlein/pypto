...
6. **M/N tiling (when `m < M` or `n < N`)** — the `[M, N]` output Acc overflows L0c. For a **plain `tile.matmul` whose result is consumed by exactly one 2D `tile.store(c, base, out)`**, the pass unrolls the output into a `ceil(M/m) × ceil(N/n)` grid: for each sub-tile origin `(mi, ni)` it computes the `[m, n]` (partial on the boundary, `min(m, M-mi) × min(n, N-ni)`) sub-tile and emits `tile.store(c_sub, [base_r + mi, base_c + ni], out_prev)`. When **K spans ≥ 2 L0 blocks**, each sub-tile is an independent **pipelined K-loop** (the `[m, K]`/`[K, n]` panel does not fit L0, so it is re-extracted per sub-tile). When **`k == K`** (the full K fits L0a/L0b at once), the whole operand panel fits L0, so the grid is emitted as a serpentine **snake** that keeps one panel resident and **reuses it across sub-tiles** — issuing only `P·Q + 1` operand extracts (`P = ceil(M/m)`, `Q = ceil(N/n)`) instead of `2·P·Q`. The pass auto-picks a **row snake** (A-stationary — reuse the `[m, K]` left panel down each N-sweep) or a **column snake** (B-stationary — reuse the `[K, n]` right panel) by keeping the *more expensive* panel resident (`T_row − T_col = (P−1)(Q−1)(B_cost − A_cost)`, `A_cost = m·K·bytes_a`, `B_cost = K·n·bytes_b`); reversing the inner sweep each step also reuses the moving panel across every turn. The stores chain the output tensor in SSA form; the final store's result replaces the original store downstream. Boundary sub-tiles use static partial extents (no `valid_shape` masking needed) — `m`/`n` need not divide `M`/`N`. The following M/N regimes are **deferred** and emit `PH-AT-006` (the matmul is left untouched): `tile.matmul_acc` (needs per-sub-tile slicing of the caller's `[M, N]` accumulator), a `Vec` left operand (PV path), and a result not consumed by a single 2D store (needs the Mat-scratch / `tile.assemble` path for on-chip consumers).
...
| `PH-AT-006` | Output exceeds L0c but the M/N direct-store fold is not applicable — `tile.matmul_acc`, a Vec left operand, or a result not consumed by a single 2D `tile.store` (the Mat-scratch / `tile.assemble` path for on-chip consumers is deferred) |
