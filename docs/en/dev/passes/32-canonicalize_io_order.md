...
A loop carrying `pipeline_double_buffer_c=true` uses the **same** compute/store key as any other pipeline, so a depth-4 source pipeline emits `M0 S0 M1 S1 M2 S2 M3 S3`. That order *is* the dbC ping-pong: each drain is issued directly after the MAD that produced it and runs while the next MAD executes on the other accumulator. The attribute's only effect here is that cube-produced Acc memberships are rotated modulo 2 after scheduling, so operand memberships retain depth 4 while `MemoryReuse` preserves exactly two L0C buffers.

> Until the dbC emit was corrected, this key grouped stages by `stage/2` and emitted `M0 M1 S0 S1 M2 M3 S2 S3`. That deferred every drain past the compute it was meant to overlap. It was there to force the accumulator live ranges to overlap so an allocator would separate them — but separation is carried by the rotated membership, so the reorder bought nothing and cost the overlap. An Ascend 910B2 campaign measured the reorder alone at ≤ 0.08 µs.
