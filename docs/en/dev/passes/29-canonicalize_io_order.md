...
- [`MaterializeTensorStrides`](30-materialize_tensor_strides.md) — fills implicit `TensorView` strides after that cleanup and before `InitMemRef` consumes them
- [`MemoryReuse`](33-memory_reuse.md) — runs after this pass; enforces stage buffer separation explicitly via `pipeline_membership` (this pass only shapes the schedule)
