...
5. **Build the K-loop** (per output sub-tile — the whole output when K-only, or each `[m, n]` sub-tile when M/N tiling):
   ...
   - **Full-output K fallback** — the minimum-wall choice may split M or N merely to fit an operand even though the complete `[M, N]` accumulator fits L0C. If the result has an elementwise/vector consumer (for example QK → softmax), no M/N placer can reconstruct that consumer's complete tile. The pass re-runs the chooser with `m=M` and `n=N`, leaving only K as a tiling axis. This keeps the complete result on chip while ensuring each pipelined Left/Right panel fits its architectural buffer; it is not used when the complete output itself exceeds L0C.
6. **M/N tiling (when `m < M` or `n < N`)** — the selected minimum-wall plan needs spatial output tiling, either for L0 capacity or operand-feed cost.
...
| `tile.matmul` whose complete output fits L0c but the minimum-wall tile splits M/N and the result has a non-placeable elementwise/vector consumer | Re-chooses with full M/N fixed and emits a K-only loop, so oversized operand panels are not left un-tiled |
| `tile.matmul` (plain, Mat left, Mat right) whose selected plan splits M/N, consumed by one 2D `tile.store` | M/N-tiled: `ceil(M/m) × ceil(N/n)` grid of sub-tile K-loops, each stored straight to the output (direct-store) |
