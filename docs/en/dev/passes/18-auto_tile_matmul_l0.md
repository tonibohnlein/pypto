...
For a canonical split-K chain, the same grid encloses the **source** reduction rather than slicing its final Acc. A single-Acc (`dbC=1`) choice completes and drains each output tile before starting the next. A double-buffered (`dbC=2`) choice alternates uniform output tiles over two persistent Acc seeds and groups two complete source reductions before their two drains, realizing the two-Acc ping-pong without interleaving or changing either reduction. The ordinary rewrite of calls inside each narrowed reduction still chooses legal inner-K blocking. In issue #2232, the logical INT32 `[16, 1152]` result occupies `32 × 1152 × 4 = 144 KiB` physically on Ascend910B, so it is split along N. Each generated N tile runs all eight source K blocks; dbC determines whether those completed reductions are drained singly or in pairs.
