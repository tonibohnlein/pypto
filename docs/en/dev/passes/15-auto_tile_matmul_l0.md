...
1. **Filter** — operand layout: `(lhs, rhs)` for `tile.matmul`, `(acc, lhs, rhs)` for `tile.matmul_acc`, and `(lhs, rhs, bias)` for `tile.matmul_bias`. Both matrix operands must be `Var`/`IterArg` (via `AsVarLike`) of `TileType` with static 2D shape. The right (B) operand must be `memory_space == Mat`; the left (A) operand may be `Mat` or `Vec`. A bias must be a static `[1, N]` Mat/Bias tile. An already-Bias-resident source is reusable when N stays whole; N tiling requires the normal pre-inference Mat source because the ISA has no Bias-to-Bias sub-window extract. Other cases (a Vec right operand, dynamic shapes, or a malformed bias) are skipped silently.
...
5. **Build the K-loop** (per output sub-tile — the whole output when K-only, or each `[m, n]` sub-tile when M/N tiling):
   ...
   - `tile.matmul_bias` — follows the fresh-accumulator path, but extracts the matching `[1, n]` bias window and uses `tile.matmul_bias` exactly once on the first K block; later blocks use `tile.matmul_acc`.
   ...
   - **Vec left operand staging** — when the left (A) operand is `Vec`-resident (PV / `score·V`), a single `tile.move(lhs, target_memory=Mat)` is emitted **before** the K-loop or M/N grid and the Left extracts slice from that staged Mat tile. The pass declines M/N tiling if the complete staged physical tile, together with compiler-created Mat materialization, cannot fit Mat capacity. Keeping the Vec→Mat crossing a `tile.move` lets [`ExpandMixedKernel`](20-expand_mixed_kernel.md) recognise it (`CollectCVBoundaryMoves` only matches `tile.move`) and lower it to the cross-core `tpop_from_aiv` handshake.
...
6. **M/N tiling (when `m < M` or `n < N`)** — the physical `[M, N]` output Acc footprint overflows L0c.
   ...
   For a **fresh `tile.matmul` or `tile.matmul_bias` whose result is consumed by exactly one 2D `tile.store(c, base, out)`**, the pass tiles the output into a `ceil(M/m) × ceil(N/n)` grid: for each sub-tile origin `(mi, ni)` it computes the partial-aware sub-tile and emits `tile.store(c_sub, [base_r + mi, base_c + ni], out_prev)`. When **K spans ≥ 2 L0 blocks**, each sub-tile is an independent **pipelined K-loop**. When **`k == K`**, the grid is emitted as nested loops over the divisible interior so [`LowerPipelineLoops`](26-lower_pipeline_loops.md) double-buffers the moving operand. Bias windows are broadcast over M and sliced by N. The outer loop owns the stationary panel; output-stationary versus A/B-stationary loop order follows the chooser's design point. The L-shaped partial boundary is peeled into straight-line partial tiles, so `m`/`n` need not divide `M`/`N`. The stores chain the output tensor in SSA form; the final store's result replaces the original store downstream.
   ...
   A **linear accumulator chain** starts with a fresh `tile.matmul` / `tile.matmul_bias` and follows one-use top-level `tile.matmul_acc` accumulator edges. It is rewritten atomically: each `[m, n]` output sub-tile completes every stage's full K reduction before being placed. This supports compiler-visible reduction chains without slicing a caller-owned Acc. An arbitrary standalone `tile.matmul_acc` with an opaque caller-owned accumulator remains **deferred** with `PH-AT-006`.

   **Placement (direct-store, Mat-scratch, or both).** Every grid hands each `[m, n]` Acc sub-tile to a `SubtilePlacer`. The **`DirectGmPlacer`** stores it to the programmer's DDR output. The **`MatScratchPlacer`** keeps the whole logical result on-chip in an L1/**Mat** scratch assembled from the sub-tiles. The pass selects Mat-scratch when all on-chip uses are matmul-operand reads and the scratch's backend-aware physical footprint fits Mat capacity. A compatible f32→bf16/f16 `rint` cast is absorbed into this FIXPIPE writeback. When the source result is **both stored and reused**, a composite placer fans each Acc sub-tile to the existing GM store and the compiler-created Mat scratch; consumers must occur after the store where this atomic materialization is emitted. The source program still expresses only its logical store and dataflow—it does not need to contain `tile.assemble`. Mixed/non-matmul on-chip consumers, including an Acc→Vec move, remain deferred because this pass does not invent a new cross-core materialization protocol.
...
**Tests**: `tests/ut/ir/transforms/test_auto_tile_matmul_l0.py`, `tests/ut/ir/transforms/test_l0_tile_chooser.py`, `tests/st/runtime/ops/test_auto_tile_matmul.py`
...
| Fresh `tile.matmul` / `tile.matmul_bias` whose output exceeds L0c, consumed by one 2D `tile.store` | M/N-tiled: `ceil(M/m) × ceil(N/n)` grid of sub-tile K-loops, each stored straight to the output (direct-store); a bias is N-sliced and applied once per sub-tile |
| Fresh matmul-family result whose output exceeds L0c, consumed *entirely* as a matmul operand, and whose physical `[M, N]` scratch fits Mat/L1 | M/N-tiled into an L1/**Mat** scratch (per-sub-tile Acc→Mat `tile.assemble`), kept on-chip for the consumer (Mat-scratch) |
| Fresh result that is both stored and reused by later matmul operands | M/N-tiled once and materialized atomically to both the existing GM store and a compiler-created Mat scratch |
...
| Linear fresh-matmul → one-or-more `tile.matmul_acc` chain whose output exceeds L0c | M/N-tiled at chain scope; each output sub-tile completes all chain stages before placement |
...
| Output exceeds L0c but no M/N placement applies — standalone caller-owned `matmul_acc`, mixed/non-matmul on-chip consumer, consumer-before-store composite, or a scratch/staged Vec operand whose physical footprint exceeds Mat/L1 | Skipped with `PerfHint` (`PH-AT-006`) |
...
| `PH-AT-006` | Output exceeds L0c but no supported transparent M/N placement applies. This covers a caller-owned `tile.matmul_acc` outside a canonical/linear compiler-visible chain, mixed or non-matmul on-chip consumers, a consumer that precedes the existing GM-store materialization point, and a Mat scratch or staged full Vec operand whose physical footprint cannot fit Mat/L1. The canonical split-K case from issue #2232 does not emit this hint. |
