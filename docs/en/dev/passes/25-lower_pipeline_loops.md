...
Hierarchical cube schedules mark their outer K-window loop with
`pipeline_gm_to_l1_only`. If `AutoTileMatmulL0` has already attached an inner
membership to a Left/Right/Bias operand, the outer loop preserves that inner
two-slot ring instead of multiplying it into four L0 buffers. If the child fits
one L0 tile and therefore has no inner membership, the outer loop supplies the
required two-slot L0 operand ring itself. Acc remains single-buffered in both
cases. Dropping the outer membership when no inner one exists would alias
adjacent stages onto one live L0A/L0B address.
