...
- **Before memory planning** (after `MaterializeTensorStrides`, before
  `InitMemRef`): removes statically dead branches left by pipeline-slot
  substitution so unreachable accesses do not contribute lifetimes or DSA
  reuse-penalty candidates.
- **Near the end of the tile pipeline** (after `MaterializeDistTensorCtx`, before
  `LegalizeGraphBoundary`): final cleanup of folds exposed by memory-space
  inference, layout resolution, and later host/distributed lowering.
...
- Before `InitMemRef` so allocation analyses observe only reachable tile accesses.
- Near the end of the tile pipeline as a cleanup pass so that downstream artifacts (printed IR, codegen) are not littered with `K + 0` or `idx * 1` residue.
