...
| A9 | a singleton-column reduction may use row-reduction geometry only when the complete scheduled group is coordinate-equivalent | record `singleton_column_to_row`, keep the original tensor ABI, emit a zero-copy `[N,1]→[1,N]` view, and replay column reductions/expansions as row operations | ✅ scalar reduction outputs are supported. Full-frame descendants that cannot cross the coordinate boundary safely decline, and transformed AutoFuse groups are kept whole. A full-frame singleton axis receives the physical DMA frame required by its layout, while a true broadcast or collapsed reduction axis stays thin. The orthogonal axis still follows its tile layout: an FP32 column-broadcast strip may be physical `[8,1]` with valid shape `[4,1]`, and a row-reduction scalar is physical `[8,1]` with logical `valid_shape=[1,1]`. |
