...
def test_canonical_split_k_output_grid_realizes_dbc(planner):
    """The enclosing split-K rewrite realizes the chooser's dbC grid.

    This Qwen-like shape selects four 16x64 output tiles. The unrolled-grid
    emitter gives them one unique pipeline group and alternating stages, so
    both in-tree DSA-RP and PTOAS preserve the intended two-slot ping-pong.
    """
    ...
    assert printed.count('"pipeline_membership": "2097152:0"') == 2
    assert printed.count('"pipeline_membership": "2097152:1"') == 2
    for n_offset in (0, 64, 128, 192):
        assert f"[0, {n_offset}]" in printed
    if planner == passes.MemoryPlanner.PTOAS:
        assert 'pl.MemRef("acc__tile_dbc_slots", slots=2)' in printed
