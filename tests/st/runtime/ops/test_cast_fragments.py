...
import pytest
import torch
from harness import DataType, TensorSpec, st
...
DYNAMIC_VALID_COLS = [0, 1, 127, 128, 129, 223, 224, 255, 256, 257, 384]


def dynamic_cast_stage_program(cols: int, source_dtype, target_dtype, mode: str):
    """One binary whose logical width is supplied at runtime by orchestration."""

    @pl.program
    class DynamicCastStageProgram:
        ...
        def kernel(
            ...
            tile = pl.load(value, [0, 0], [32, cols], valid_shape=[valid_rows, valid_cols])
            converted = pl.cast(tile, target_type=target_dtype, mode=mode)
            return pl.store(converted, [0, 0], output)
        ...
        def main(
            ...
            return self.kernel(value, output, valid_rows, valid_cols)

    return DynamicCastStageProgram
...
def dynamic_stage_cases():
    stages = [
        ("i32_f16", pl.INT32, pl.FP16, "round", torch.int32, torch.float16, DataType.INT32, DataType.FP16),
        ("f16_i8", pl.FP16, pl.INT8, "trunc", torch.float16, torch.int8, DataType.FP16, DataType.INT8),
    ]
    cols = 384
    rows = 16
    for valid in DYNAMIC_VALID_COLS:
        for name, src_pl, dst_pl, mode, src_torch, dst_torch, src_dt, dst_dt in stages:
            generator = torch.Generator().manual_seed(valid)
            integers = torch.randint(-127, 128, (32, cols), generator=generator, dtype=torch.int32)
            value = integers.to(src_torch)
            valid_shape = torch.tensor([rows, valid], dtype=torch.int64)
            output = torch.full((32, cols), -128, dtype=dst_torch)
            expected = output.clone()
            expected[:rows, :valid] = value[:rows, :valid].to(dst_torch)
            yield st.case(
                dynamic_cast_stage_program(cols, src_pl, dst_pl, mode),
                name=f"cast_dynamic_{name}_{rows}_{valid}",
                tensors=[
                    TensorSpec("value", [32, cols], src_dt, init_value=value),
                    TensorSpec("valid_shape", [2], DataType.INT64, init_value=valid_shape),
                    TensorSpec("output", [32, cols], dst_dt, init_value=output, is_output=True),
                ],
                golden=lambda _, expected=expected: expected,
                compare=valid_rectangle(rows, valid),
            )
...
@st.cases(*dynamic_stage_cases())
def test_cast_stage_dynamic_tail(case_run):
    case_run.assert_passed()
